...
//! `S = R + e·P` in advance and use it as the encryption point for the matching CET. When the
//! oracle reveals the signature `(R, s)`, `s` is the discrete log of `S`, which decrypts the CET.
//!
...
//! wrap libsecp256k1-zkp. The only thing this module owns is the byte-level bridge between
//! `hunch-protocol`'s `secp256k1` 0.30 types and `ddk-dlc`'s `secp256k1-zkp` types, plus the
//! canonical message (identical bytes to `hunch_protocol::oracle::OracleAttestation`).
//!
...
/// `hunch/oracle/v1\n<market>\n<outcome>` message. Keeping this in lockstep with the protocol
/// crate is what makes a `ddk-dlc`-produced signature verify under `hunch-protocol`'s
/// `secp256k1` 0.30 verifier.
fn attestation_digest(market: &str, outcome: Outcome) -> [u8; 32] {
...
/// Computes the per-outcome signature point `S = R + e·P` (what the mint encrypts the CET to).
///
/// `oracle_pubkey` and `nonce_pubkey` are x-only hex (32 bytes). Returned as 33-byte
/// compressed-point hex. Exposed so the CLI / mint side can verify announce↔attest binding.
...
mod tests {
    ...
    fn xonly_hex(secret_hex: &str) -> String {
    ...
    fn attestation_with_nonce_verifies_under_protocol_verifier() {
        ...
        // The signature must verify under hunch-protocol's (secp256k1 0.30) verifier, proving
        // the ddk-dlc signature is a standard BIP-340 signature over our canonical message.
        let att = OracleAttestation {
            market: market.clone(),
            outcome: Outcome::Yes,
            signature_hex: sig_hex,
        };
        let oracle_xonly = secp256k1::XOnlyPublicKey::from_slice(
            &hex::decode(xonly_hex(ORACLE_SECRET)).unwrap(),
        )
        .unwrap();
    ...
    fn signature_s_equals_sig_point_dlog() {
        // The core DLC property: s·G == S = R + e·P. If this holds, the oracle's revealed `s`
        // is exactly the secret that decrypts the CET adaptor-encrypted to S.
        let secret: [u8; 32] = hex::decode(ORACLE_SECRET).unwrap().try_into().unwrap();
        let nonce: [u8; 32] = hex::decode(NONCE_SECRET).unwrap().try_into().unwrap();
        let market = format!("{}:30888:btc-100k", "aa".repeat(32));
    ...
    fn different_nonces_give_different_signatures() {
        ...
        n2[0] ^= 0x01;
        let market = "m";
        let s1 = sign_attestation_with_nonce(&secret, &n1, market, Outcome::Yes).unwrap();
        let s2 = sign_attestation_with_nonce(&secret, &n2, market, Outcome::Yes).unwrap();
        assert_ne!(s1, s2);
    }
}
