...
fn active_keyset(client: &reqwest::blocking::Client, url: &str) -> (Id, PublicKey) {
    ...
    let pk_hex = keyset["keys"]["1"]
        .as_str()
        .expect("mint must offer the 1-sat denomination");
...
fn e2e_mint_lock_and_redeem_against_live_mint() {
    ...
    yes_proof
        ...
        .unwrap();
    assert!(
        try_swap(&client, &url, keyset_id, yes_proof),
        "mint must accept the YES token after the YES attestation"
    );
    ...
    no_proof
        ...
        .unwrap();
    assert!(
        !try_swap(&client, &url, keyset_id, no_proof),
        "mint must reject the NO token signed with the YES key"
    );
